...
const io = new Server(server, {
  cors: {
    origin: process.env.NODE_ENV === "production"
      ? process.env.FRONTEND_URL   // ✅ only your frontend (e.g., https://yourapp.vercel.app)
      : "http://localhost:3000",
    methods: ["GET", "POST"],
    credentials: true,
  },
...
// Middleware to ensure DB is connected before API use
const dbCheck = (req, res, next) => {
  if (mongoose.connection.readyState !== 1) {
    return res.status(503).json({ message: "Database not available" });
  }
...
io.on("connection", (socket) => {
  ...
  socket.on("send_message", (messageData) => {
    ...
    socket.to(messageData.receiverId).emit("receive_message", messageData);
    console.log(`📨 Message from ${messageData.sender} ➝ ${messageData.receiverId}`);
  });
